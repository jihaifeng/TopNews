...
/**
 * Func：图片加载帮助类
 * Desc: 不加 dontAnimate()，有的机型会出现图片变形的情况，先记下找到更好的方法再处理
 * Author：jihf
 * Data：2017-03-08 16:11
...
public class GlideUtils {
  ...
  public static void loadFit(Context context, String url, ImageView view, int defaultResId) {
    if (PreferencesUtils.isShowImageAlways(context) || NetWorkUtils.isWifiConnected(context)) {
   ...
   * @param height
   */
  public static void loadFitOverride(Context context, String url, ImageView view, int defaultResId, int width,
      int height) {
    if (PreferencesUtils.isShowImageAlways(context) || NetWorkUtils.isWifiConnected(context)) {
      Glide.with(context)
          .load(url)
